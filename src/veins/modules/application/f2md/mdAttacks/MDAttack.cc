...
//MDAttack::MDAttack() {
//}
MDAttack::MDAttack() : vp(nullptr) {  // Initialize vp to nullptr
}

void MDAttack::init(attackTypes::Attacks myAttackType, double MaxRandomPosX,double MaxRandomPosY,F2MDParameters *params, VehiclePatterns* vp) {

    this->params = params;
    this->vp = vp; // Initialize the pointer to VehiclePatterns
    StopInitiated = false;
    DoSInitiated = false;
...
//    }
    ...
    InfoSeq = 0; //record info sequence
}
...
//             std::cout<<"YOffset:"<<YOffset<<"\n";
    ...
    case attackTypes::DataReplaySybil: {
        ...
        break;
    //mimic maneuvering attack
    case attackTypes::mimic: {
        const auto& laneChangeInfoList = vp->getLaneChangeInfoList();
        // Ensure that InfoSeq is within bounds
        if (laneChangeInfoList.empty()) {
            std::cerr << "Error: laneChangeInfoList is empty" << std::endl;
            break; // Exit the case to avoid undefined behavior
        }

        const auto& attackInfo = laneChangeInfoList[InfoSeq];
//        // 获取当前保存的信息
//        LaneChangeInfo attackInfo = laneChangeInfoList[0];
//
//        // 移除已经使用的信息
//        laneChangeInfoList.erase(laneChangeInfoList.begin());
        ...
        attackBsm.setSenderSpeed(attackInfo.speed);
        attackBsm.setSenderSpeedConfidence(*curSpeedConfidence);

        attackBsm.setSenderAccel(attackInfo.acceleration);
        attackBsm.setSenderAccelConfidence(*curAccelConfidence);

        attackBsm.setSenderHeading(attackInfo.heading);
        attackBsm.setSenderHeadingConfidence(*curHeadingConfidence);

        // 设置其他 BSM 字段（如位置、宽度、长度等）
        attackBsm.setSenderWidth(*myWidth);
        attackBsm.setSenderLength(*myLength);

        InfoSeq++;

        if (InfoSeq >= static_cast<int>(laneChangeInfoList.size())) {
            InfoSeq = 0;
        }
    }
