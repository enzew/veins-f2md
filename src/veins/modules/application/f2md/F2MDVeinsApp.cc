...
<<<<<<< HEAD
        params.enableMTA = par("enableMTA"); //enable maneuver triggered attacks
        params.enableDT = par("enableDT");   //enable detect turning
        params.selectAttacker = par("selectAttacker"); //enable select attacker involving maneuvers
=======
        params.enableMTA = par("enableMTA");
	params.enableDT = par("enableDT");
>>>>>>> 7aacfa11e3e30926eff0d1252549e14694c2ee77
    ...
    if (stage == 0) {
        ...
        //record timer initialization
        recordLaneChangeEvent = new cMessage("recordLaneChangeEvent");
    ...
    double globalAttacker)
{
    ...
    if (localAttacker > realFactor) {
        //special attack mimic
        if (myAttackType == attackTypes::mimic){
            //if the records are empty
            if (vp.getLaneChangeInfosize() < 2){
                return mbTypes::Genuine;
            }
        }
...
<<<<<<< HEAD
    std::string SendercurrentLaneId = bsm->getLaneId(); //get lanid from bsm

    //check distance between me and sender
    MDMLib mdmLib = MDMLib();
    double distance = mdmLib.calculateDistancePtr(&curPosition, &bsm->getSenderPos());
    // check and update laneid
    if (vehicleLaneMap.find(senderPseudonym) != vehicleLaneMap.end()) {
          std::string SenderpreviousLaneId = vehicleLaneMap[senderPseudonym];
          if (SenderpreviousLaneId != SendercurrentLaneId) {
              // Sender lane changed
              std::cout << myPseudonym << " Report: " << "Vehicle " << senderPseudonym << " has changed lanes from "
                        << SenderpreviousLaneId << " to " << SendercurrentLaneId << std::endl;
              // enabled maneuver triggered attacks
              if (params.enableMTA){
                  //aiming at maneuvering vehicles
                  if (!params.selectAttacker && myMdType == mbTypes::LocalAttacker){

                      if (distance < 20.0){
                          //become an attacker if the vehicles under radius of 20.0 are changing lane
...
//                                  myMdType = mbTypes::Genuine;
                                  ...
                                  //aiming at maneuvering vehicle with MTA on, mimic and enough data recorded
                                  //add the lane changing vehicle to target node
                                  addTargetNode(senderPseudonym);
                                  treatAttackFlags();

                                  }
                          }
                          //not mimic attack type
                          else if (!laneChanged){
                              //add the lane changing vehicle to target node
                              addTargetNode(senderPseudonym);
                              //become a local attacker
                              //myMdType = mbTypes::LocalAttacker;
                              //myAttackType = params.LOCAL_ATTACK_TYPE;
                              treatAttackFlags();
                          }
      ...
      vehicleLaneMap[senderPseudonym] = SendercurrentLaneId;
=======
...
//            traciVehicle->setColor(TraCIColor(255, 20, 147, 255)); //deep pink
//        }
//        else {
...
>>>>>>> 7aacfa11e3e30926eff0d1252549e14694c2ee77
        ...
        if (myMdType == mbTypes::LocalAttacker) {
            //if it is mimic type
            if (myAttackType == attackTypes::mimic){
                //if the records are not enough
                if (vp.getLaneChangeInfosize() < 2){
//                    myMdType = mbTypes::Genuine;
                    EV_INFO << "Attacker doesn't have enough data for mimic" ;
                    traciVehicle->setColor(TraCIColor(0, 255, 0, 255)); // Green
                }
                else {
...
<<<<<<< HEAD
            ...
            else if (myAttackType != attackTypes::mimic){
                // just local attacker
                traciVehicle->setColor(TraCIColor(255, 0, 0, 255)); // Red
            }
        }
            ...
            if (laneChanged) {
                // lane changed
                traciVehicle->setColor(TraCIColor(0, 255, 255, 255)); // Light Cyan
            }
//
//            else if (isTargetNode(myPseudonym)) {
//                // 目标节点颜色：黄色
//                traciVehicle->setColor(TraCIColor(255, 255, 0, 255)); // Yellow
//            }
            // accused as attacker
            else if (isAccusedNode(myPseudonym)) {
                // falsely accused
                traciVehicle->setColor(TraCIColor(255, 255, 0, 255)); // Yellow
            }
            else {
...
{
    //record timer received and call record function
    if(msg == recordLaneChangeEvent)
    {
        recordLaneChangeInfo();
        treatAttackFlags();
    }else{
...
<<<<<<< HEAD
//    veins::Coord mySpeed = curSpeed; //get speed
//    veins::Coord myAcceleration = curAccel; //get acceleration
//    veins::Coord myHeading = curHeading; //get heading
=======
>>>>>>> 7aacfa11e3e30926eff0d1252549e14694c2ee77
    ...
    if (laneChanged) {
<<<<<<< HEAD
        //set maneuver color as light Cyan
        traciVehicle->setColor(TraCIColor(0, 255, 255, 255));

        //call recoding function
        recordLaneChangeInfo();
        scheduleAt(simTime() + beaconInterval, recordLaneChangeEvent);
        //maneuver triggered attacks is on
        if (params.enableMTA){
            if (params.selectAttacker && myMdType == mbTypes::LocalAttacker){
                // Perform attacks when doing lane change
//                if (params.enableMTA) {
...
//            }
//            else{
//                //if you are lane changing you can not be an attacker.
//                myMdType = mbTypes::Genuine;
//
//            }
        }
...
<<<<<<< HEAD
        // turning
        if (params.enableDT && myMdType == mbTypes::LocalAttacker) {
            //myAttackType = params.LOCAL_ATTACK_TYPE;
//            std::cout
//                << "=+#=+#=+#=+#=+#=+#=+#=+#+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+# "
//                << "\n";
//            std::cout
//                << "=+#=+#=+#=+#=+#=+# Maneuver Triggered ATTACKER =+#=+#=+#=+#=+# "
//                << myPseudonym << " : "
//                << attackTypes::AttackNames[myAttackType] << "\n";
//            std::cout
//                << "=+#=+#=+#=+#=+#=+#=+#=+#+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+#=+# "
...
<<<<<<< HEAD
void JosephVeinsApp::recordLaneChangeInfo() {

    //record features

    if (vp.getLaneChangeInfosize() < 2){
        vp.addLaneChangeInfo(curSpeed, curAccel, curHeading);
        recordcounter++;
...
//        }
    ...
    EV_INFO << "Recorded lane change info of" <<"" << myPseudonym
            << "Speed: " << curSpeed
            << ", Acceleration: " << curAccel
            << ", Heading: " << curHeading << endl;
}


//initialization of Attack
void JosephVeinsApp::initAttack() {
...
<<<<<<< HEAD
    mdAttack.init(myAttackType, MaxRandomPosX, MaxRandomPosY, &params, &vp);
    ...
    int serial)
{
    ...
    if (BasicSafetyMessage* bsm = dynamic_cast<BasicSafetyMessage*>(wsm)) {
        ...
        // 输出 speed, acceleration, heading 信息
        EV << "BSM Info of vehicle" << myPseudonym <<" \n";
        EV << "Speed: " << curSpeed << "\n";
        EV << "Acceleration: " << curAccel << "\n";
        EV << "Heading: " << curHeading << "\n";
        //F2MD
